...
`,F=f(W)`
  ${({$tabletWidth:i})=>{const{tokens:t}=p(),{modal:e}=t,{width:n}=L(),s=n-parseInt(e.padding.tablet,10)*2;return u`
    ...
    `}}
`;function z(i,t){a.useEffect(()=>{const e=i.current;e&&(t&&!e.open?(e.showModal(),e.focus()):!t&&e.open&&e.close())},[t])}function A(i,t){a.useEffect(()=>{const e=i.current;function n(s){s.key==="Escape"&&(s.preventDefault(),t==null||t())}if(e)return e.addEventListener("keydown",n),()=>{e.removeEventListener("keydown",n)}},[t])}function S(){return/android|ipad|iphone/i.test(navigator.userAgent)}function X(i){const{isOpen:t,onRequestClose:e,children:n,testID:s="modal",tabletWidth:l,...d}=i,o=a.useRef(null),h=a.useRef(null),[m,E]=a.useState();function j(w){w&&E(w)}z(o,t),A(o,e),a.useEffect(()=>{if(!t||S())return;const w=requestAnimationFrame(()=>{const b=o.current,g=h.current;b&&document.activeElement===b?(b.querySelector('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])')??g??b).focus():g&&!(b!=null&&b.contains(document.activeElement))&&g.focus()});return()=>cancelAnimationFrame(w)},[t]);const y=w=>{w.stopPropagation()};H({isDisabled:!t});let $=c.jsx(C,{...d,ref:o,"aria-modal":!0,tabIndex:-1,"data-testid":s,onClick:e,children:c.jsxs(F,{ref:h,tabIndex:-1,onClick:y,$tabletWidth:l,children:[n,c.jsx("div",{ref:j})]})});return S()||($=c.jsx(R,{contain:t,restoreFocus:!0,children:$})),c.jsx(I,{portalElement:m,children:$})}const D=f.div`
  position: relative;
  height: 100%;
  overflow: hidden;
`,k=f.div`
...
`,O=f.div`
  ${()=>{const{tokens:{scrollShadowContainer:i,scrollBar:{thumbColor:t,trackColor:e}}}=p();return u`
    ...
    `}}
`;function P(i){const t=a.useRef(null),e=a.useRef(null),[n,s]=a.useState(!1),[l,d]=a.useState(!1);return a.useEffect(()=>{i==null||i({topShadowVisible:n,bottomShadowVisible:l})},[i,n,l]),a.useEffect(()=>{if(!t.current||!e.current)return;const o=new IntersectionObserver(([m])=>{m&&s(!m.isIntersecting)}),h=new IntersectionObserver(([m])=>{m&&d(!m.isIntersecting)});return o.observe(t.current),h.observe(e.current),()=>{o.disconnect(),h.disconnect()}},[]),{topShadowRef:t,bottomShadowRef:e,topShadowVisible:n,bottomShadowVisible:l}}function Z(i){const{tabIndex:t,onChange:e,children:n}=i,{topShadowRef:s,bottomShadowRef:l,topShadowVisible:d,bottomShadowVisible:o}=P(e);return c.jsxs(D,{children:[c.jsx(B,{$visible:d,"aria-hidden":!0}),c.jsxs(O,{tabIndex:t,children:[c.jsx("div",{ref:s}),n,c.jsx("div",{ref:l})]}),c.jsx(M,{$visible:o,"aria-hidden":!0})]})}export{W as C,X as M,Z as S};
